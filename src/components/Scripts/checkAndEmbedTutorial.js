...
const tutorialIndexPath = path.join(
  __dirname,
...
if (!fs.existsSync(tutorialIndexPath)) {
  console.log(
    "tutorialIndex.json not found. Generating tutorial embeddings...",
  );
  execSync(`node ${embedScriptPath}`, { stdio: "inherit" });
} else {
  console.log(
    "tutorialIndex.json already exists. Skipping embedding generation.",
  );
}
