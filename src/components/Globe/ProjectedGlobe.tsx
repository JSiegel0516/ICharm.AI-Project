...
import { ProjectedProjection } from "./ProjectedProjection";
import {
  WinkelBoundaries,
...
import { createProjection } from "./projectionConfig";
...
const orientationsEqual = (a?: MapOrientation, b?: MapOrientation) => {
  if (!a || !b) return false;
  return (
...
const ProjectedGlobe: React.FC<Props> = ({
  projectionId,
  rasterGridData,
  rasterGridKey,
  rasterGridDataKey,
  ...
  const projectionRef = useRef<ProjectedProjection | null>(null);
  ...
  const renderMesh = useCallback(() => {
    if (
      ...
      console.log("[ProjectionMeshWorker] renderMesh skipped", {
    ...
    if (!rasterGridData || !currentDataset?.colorScale?.colors?.length) {
      meshVisibleRef.current = false;
      setMeshVisible(false);
      console.log("[ProjectionMeshWorker] renderMesh missing raster/color");
      pendingMeshRenderRef.current = true;
      return;
    }
    if (
    ...
    if (scale < MIN_SCALE_FOR_MESH) {
      meshVisibleRef.current = false;
      setMeshVisible(false);
      console.log("[ProjectionMeshWorker] renderMesh below scale", scale, {
    ...
    console.log("[ProjectionMeshWorker] renderMesh post", {
      width: payload.width,
  ...
  const scheduleMeshRender = useCallback(
    (force = false) => {
      ...
      console.log("[ProjectionMeshWorker] scheduleMeshRender", {
        projectionId,
      });
  ...
  useEffect(() => {
    ...
    const worker = new Worker(
      new URL("./projectionMeshWorker.ts", import.meta.url),
      { type: "module" },
    );
    console.log("[ProjectionMeshWorker] created", { projectionId });
    worker.onmessage = (event: MessageEvent) => {
      if (event.data?.type === "debug") {
        console.log("[ProjectionMeshWorker]", event.data);
        if (event.data?.stage === "pong" && !meshWorkerReadyRef.current) {
          ...
          console.log("[ProjectionMeshWorker] init posted", { projectionId });
    ...
    worker.onmessageerror = (event) => {
      console.error("[ProjectionMeshWorker] message error", event);
    };
    worker.onerror = (error) => {
      console.error("[ProjectionMeshWorker] error", error);
    };
