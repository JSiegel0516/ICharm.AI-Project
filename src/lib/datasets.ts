import type { ColorScale, Dataset, DatasetBackendDetails, DatasetRasterConfig } from '@/types';
import { samplePaletteHex, generateColorScaleLabels } from '@/lib/colormaps';
...
export interface BackendDatasetRecord {
  ...
  endDate?: string | null;
  supportsRaster?: string | boolean | number | null;
...
function buildBackendDetails(
  ...
  const supportsRaster = toBoolean(record.supportsRaster);
  const colorMap = record.colorMap ?? null;
  const valueMin = toNumber(record.valueMin);
  const valueMax = toNumber(record.valueMax);
...
export function normalizeDataset(record: BackendDatasetRecord): Dataset {
  ...
  const backend = buildBackendDetails(record);
  const supportsRaster = backend.supportsRaster ?? false;
  const rangeMin = backend.valueRange?.min ?? null;
  const rangeMax = backend.valueRange?.max ?? null;
  const paletteName = backend.colorMap ?? null;

  let colorScale = cloneColorScale(baseColorScale);
  ...
  if (
    ...
    const sampleCount = baseColorScale.colors.length;
    const colors = samplePaletteHex(paletteName, sampleCount);
    const labels = generateColorScaleLabels(rangeMin, rangeMax, sampleCount);
    colorScale = {
      min: rangeMin,
      max: rangeMax,
      colors,
    ...
    if (rangeMin !== null && Number.isFinite(rangeMin)) {
      colorScale.min = rangeMin;
    }
    if (rangeMax !== null && Number.isFinite(rangeMax)) {
      colorScale.max = rangeMax;
    }
  }

  const raster: DatasetRasterConfig = {
    supportsRaster,
