...
async function seedDatasets() {
  ...
  const datasets = lines.slice(1).map((line) => {
    ...
    return {
      ...
      datasetShortName:
        row.datasetShortName == "" ? null : row.datasetShortName,
      description: row.description == "" ? null : row.description,
      layerParameter: row.layerParameter,
      statistic: row.statistic,
      datasetType: row.datasetType,
      levels: row.levels,
      levelValues: row.levelValues === "" ? null : row.levelValues,
      levelUnits: row.levelUnits === "" ? null : row.levelUnits,
      stored: row.stored,
      storageType: row.storageType,
      postgresProcessor:
        row.postgresProcessor === "" ? null : row.postgresProcessor,
      toProcess: row.toProcess,
      inputFile: row.inputFile === "" ? null : row.inputFile,
      keyVariable: row.keyVariable,
      levelVariable: row.levelVariable === "" ? null : row.levelVariable,
      units: row.units,
      spatialResolution: row.spatialResolution,
      engine: row.engine,
      kerchunkPath: row.kerchunkPath === "" ? null : row.kerchunkPath,
      infoLocation: row.infoLocation === "" ? null : row.infoLocation,
  ...
  try {
    console.log("🧹 Clearing existing datasets table...");
