...
def fix_global_surface_temperature():
    ...
    # File paths
    nc_file = Path(f"{curr_folder}/../../datasets/air.mon.anom.v6.nc")
    zarr_file = Path(f"{curr_folder}/../../datasets/global_surface_temperature.zarr")
    backup_zarr = Path(
        f"{curr_folder}/../../datasets/global_surface_temperature_backup.zarr"
    )
    ...
    try:
        ...
        for var_name in ds_fixed.data_vars:
            ...
            clean_attrs = {
                ...
                or k == "units"
            }  # Keep units for data variables, not time
        ...
        if backup_zarr.exists() and not zarr_file.exists():
            print("🔄 Restoring backup zarr file...")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("  time_series = ds.sel(time=slice(start_date, end_date))")
        print("  data = time_series.air.mean(['lat', 'lon']).values")
    else:
