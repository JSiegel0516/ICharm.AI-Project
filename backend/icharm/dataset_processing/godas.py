...
def main():
    ...
    try:
        ...
        if "time" in test_ds.dims:
            logger.info(
                f"  Time range: {test_ds.time.min().values} to {test_ds.time.max().values}"
            )
        ...
        logger.info(
            f"  Sample data has values: {sample_data.size > 0 and not all(sample_data.flat == 0)}"
        )
