...
class NetCDFtoDB:
    ...
    def _get_metadata_from_file(self, example_path: Path):
        with Dataset(example_path, "r") as nc:
            ...
            for idx, variable_name in enumerate(variable.dimensions):
                ...
                # Skip unknown dimensions (e.g., zlev with single parameter)
                # that we don't need to track
    ...
    def _populate_postgres_data_tables(self, conn):
        ...
        with conn.cursor() as cur:
            ...
            for month, day in tqdm(itertools.product(months, days), total=total_days):
                ...
                yearly_files = sorted(self.folder_path.rglob(f"*{month_day_str}.nc"))

                data_slices = []
                for file_idx, file in enumerate(yearly_files):
                    # Need to reshape the data from each file
                    ...
                    with Dataset(file, "r") as nc:
                        ...
                        # Filter to only include dimensions we care about (skip unknown ones like zlev)
                        expected_dims = [
                            self.latitude_variable_name,
                            self.longitude_variable_name,
                            self.time_variable_name,
                        ...
                        if self.level_variable_name is not None:
                            expected_dims.append(self.level_variable_name)
                        spatial_dims = [
                            d
                            for d in full_dims
                            if d != self.time_variable_name and d in expected_dims
                        ]
                        ...
                        for idx, time_dt in enumerate(times_dt):
                            ...
                            indexer = tuple(
                                (
                                    idx
                                    if axis == time_idx_loc
                                    else (
                                        0
                                        if variable.dimensions[axis]
                                        not in self.all_variable_locations
                                        else slice(None)
                                    )
                                )
    ...
    def _process_multi_level_gridbox_data(
        ...
        if level_axis is None:
            ...
            # TODO: This is NOT correct, it's messing up leap years and such.
            data_lat_lon_level = numpy.transpose(data, (lat_axis, lon_axis, level_axis))
            # assert data_lat_lon_level.shape == (n_lat, n_lon, n_levels)

            num_levels = data_lat_lon_level.shape[2]
            flat_values = data_lat_lon_level.reshape(-1, num_levels)
            value_cols = [f"value_{k}" for k in list(self.levels.keys())[0:num_levels]]
...
def main():
    if True:
        data_path = "/home/jsiegel/projectCHARM/ICharm.AI-Project/backend/datasets/daily"
    ...
    netcdf_to_db = NetCDFtoDB(
        ...
        years=[str(i) for i in range(1981, 2026)],
